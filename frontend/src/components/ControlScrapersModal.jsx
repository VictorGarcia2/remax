...
const ControlScrapersModal = ({ isOpen, onClose }) => {
  const [tipoInmueble, setTipoInmueble] = useState('casas'); // 'casas' | 'departamentos' | 'todos'
  const [zona, setZona] = useState('veracruz'); // 'veracruz' | 'tabasco'
  const [modo, setModo] = useState('rapido'); // 'rapido' | 'completo'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-xl w-full p-6 text-slate-800 relative space-y-6">
        ...
        {/* SI ESTÁ EN EJECUCIÓN: MOSTRAR PROGRESO */}
        {enEjecucion ? (
          <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 space-y-4">
            ...
            <p className="text-xs text-slate-500 text-center">
              Los scrapers se están ejecutando en modo Headless en el servidor. La base de datos y el modelo de IA se re-entrenarán automáticamente al finalizar.
            </p>
          </div>
        ) : (
          ...
          <div className="space-y-4 text-sm">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <button
                    key={item.id}
                    type="button"
                    onClick={() => setTipoInmueble(item.id)}
            ...
            <div>
              ...
              <input
                ...
                max="50"
                value={limitePaginas}
                onChange={(e) => setLimitePaginas(e.target.value)}
                className="w-full px-3 py-2 border border-slate-300 rounded-lg bg-white text-slate-800 font-medium"
              />
            </div>
...
export default ControlScrapersModal;
